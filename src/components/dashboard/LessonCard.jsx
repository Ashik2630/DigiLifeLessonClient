...
// ১. প্রপস হিসেবে `isPremiumUser` রিসিভ করা হলো
const LessonCard = ({ lesson, isPremiumUser = false }) => {

  const { title, shortDescription, category, emotionalTone, accessLevel, createdAt, userName, userImage } = lesson || {};

  // ২. লকড থাকার কন্ডিশনাল লজিক
  ...
  const currentToneStyle = toneStyles[emotionalTone] || "border-zinc-700 text-zinc-400 bg-zinc-800/20";

  return (
    <div className="inline-flex flex-col justify-between w-full md:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] m-1.5 md:m-1.5 lg:m-2 bg-[#0a1120] border border-zinc-800/80 rounded-2xl p-6 shadow-xl relative overflow-hidden transition-all duration-300 hover:border-zinc-700/50 hover:shadow-2xl hover:shadow-purple-950/10 min-h-87.5 vertical-top">
      ...
      <div className="flex items-center justify-between w-full mb-6">
        ...
        {/* ৩. এক্সেস লেভেল অনুযায়ী ডাইনামিক লাক্সারি ব্যাজ */}
        {accessLevel === "Premium" ? (
      ...
      {/* ৪. কন্টেন্ট এরিয়া (যদি লকড থাকে তবে এটি ব্লার এবং অপাসিটি কম দেখাবে) */}
      <div className={`grow flex flex-col justify-start transition-all duration-300 ${isLocked ? "blur-[3px] opacity-20 select-none pointer-events-none" : ""}`}>
        <h2 className="text-2xl font-serif font-bold text-zinc-100 tracking-wide mb-3 leading-snug line-clamp-2">
          {title}
        </h2>
      ...
      {/* ৫. নিচের মেটাডাটা এরিয়া (যদি লকড থাকে তবে এটিও ব্লার দেখাবে) */}
      <div className={`mt-auto pt-4 border-t border-zinc-800/50 flex flex-col gap-5 transition-all duration-300 ${isLocked ? "blur-[3px] opacity-20 select-none pointer-events-none" : ""}`}>
        <div className="flex">
          <span className={`text-xs font-semibold px-3 py-1.5 rounded-lg border ${currentToneStyle}`}>
            {emotionalTone}
          </span>
        </div>
      ...
      {/* ৬. প্রিমিয়াম লকড ওভারলে (শুধুমাত্র ফ্রি ইউজার এবং প্রিমিয়াম কন্টেন্ট এর ক্ষেত্রে দেখাবে) */}
      {isLocked && (
        <div className="absolute inset-0 bg-[#040712]/60 backdrop-blur-[1px] flex flex-col items-center justify-center text-center p-4 transition-all duration-300">
          <div className="bg-[#0a1120]/95 border border-zinc-800/90 p-5 rounded-xl shadow-2xl w-full max-w-60 flex flex-col items-center">
            <div className="p-2.5 rounded-xl bg-purple-500/10 border border-purple-500/20 mb-3 text-[#A855F7]">
              <Lock size={18} />
            </div>
            <h4 className="text-xs font-bold text-zinc-100 mb-1">Premium Vault Locked</h4>
            <p className="text-zinc-500 text-[10px] mb-4 leading-normal">Upgrade to unlock this digital life lesson.</p>
            <Link 
              href="/pricing" 
              className="inline-flex items-center justify-center bg-purple-600 hover:bg-purple-700 text-white text-[11px] font-bold py-2 px-4 rounded-xl w-full transition-all duration-200 shadow-[0_4px_15px_rgba(147,51,234,0.3)]"
            >
              Upgrade Now
            </Link>
          </div>
